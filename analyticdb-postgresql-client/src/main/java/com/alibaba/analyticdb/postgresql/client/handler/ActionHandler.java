...
public abstract class ActionHandler<T> {
	...
	public ActionHandler(ADBConfiguration config) {

	}

	public abstract void handle(T action) throws ADBClientException;
	...
	public static Record convertRecordColumnType(Record record) {
		...
		for (int keyIndex : record.getMetadata().getKeyIndex()) {
			Object obj = record.getObject(keyIndex);
			int type = record.getMetadata().getColumnSchema()[keyIndex].getType();
			switch (type) {
				case Types.INTEGER:
		...
		if (needConvert) {
			ret = Record.build(record.getMetadata());
			for (int keyIndex : record.getMetadata().getKeyIndex()) {
				Object obj = record.getObject(keyIndex);
				int type = record.getMetadata().getColumnSchema()[keyIndex].getType();
				switch (type) {
