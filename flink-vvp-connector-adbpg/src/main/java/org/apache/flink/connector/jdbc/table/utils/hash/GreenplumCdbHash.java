...
import static org.apache.flink.connector.jdbc.table.utils.hash.GreenplumHashFunc.dateHash;
import static org.apache.flink.connector.jdbc.table.utils.hash.GreenplumHashFunc.hashBpChar;
import static org.apache.flink.connector.jdbc.table.utils.hash.GreenplumHashFunc.hashBoolean;
...
import static org.apache.flink.connector.jdbc.table.utils.hash.GreenplumHashFunc.hashInt2;
...
import static org.apache.flink.connector.jdbc.table.utils.hash.GreenplumHashFunc.timetzHash;
...
public class GreenplumCdbHash {
    ...
    public static int getTargetSegmentId(String[] values, int[] isNulls, int[] dataType, int nKeys, int numSegments) {
        ...
        targetSeg = (int) cdbHashReduce(new UInt32 (hash), numSegments);
        return targetSeg;
    }
    ...
    public static UInt32 cdbHash(UInt32 hash, String value, int isNull, int type) {
        ...
        if (isNull == 0) {
            ...
            switch (type) {
                case Types.BOOLEAN:
                    if (value.length() != 1) {
                        throw new RuntimeException("replicate-greenplum unexpect input value");
                    }
                    hKey = hashBoolean(value.charAt(0));
                    break;
                case Types.SMALLINT:
                    /*
                     * In dts, we will get INTEGER data type while target data type
                     * is smallint. So hashInt2 may never be calculated.
                     */
                    hKey = hashInt2(Integer.parseInt(value));
                    break;
                case Types.INTEGER: // distclass = 10048
                ...
                case Types.BIGINT:
                    ...
                    break;
                case Types.REAL:
                case Types.FLOAT: // real distclass=10054
                ...
                case Types.VARCHAR:
                    /*
                     * Here, we will get dataTypeName = 'STRING', but actually data type id
                     * is 1 which dataTypeName is 'CHAR'.
                     */
                    hKey = hashBpChar(value, DEFAULT_CHARSET);
                    break;
                ...
                case Types.TIME:
                    /*
                     * In dts, we are unable to distinguish between time and timetz using
                     * rawDataType name or rawDataType id, so we take the support of timezone
                     * into account within function timeHash.
                     */
                    if (value.contains("+") || value.contains("-")) {
                        hKey = timetzHash(value, true);
                    } else {
